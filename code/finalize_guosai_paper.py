...
def main():
    ...
    parser.add_argument('--reading-pdf',default='reading.pdf')
    parser.add_argument('--output-dir',default=str(D))
    parser.add_argument('--verification-json',default='qa/verification.json')
    args=parser.parse_args()
    D=Path(args.output_dir)
    D.mkdir(exist_ok=True)
    shutil.copy2(P/args.reading_pdf,D/'药材烘干论文_正文阅读版.pdf')
    ...
    for f in P.rglob('*'):
        rel=f.relative_to(P)
        if not f.is_file() or rel.parts[0].startswith('qa') or '__pycache__' in rel.parts:continue
    ...
    q=json.loads((P/args.verification_json).read_text(encoding='utf8'))
    reading_pages=q['pdfs']['reading']['pages']
    full_pages=q['pdfs']['main']['pages']
    ...
    (D/'交付核验.json').write_text(json.dumps({'files':manifest,'table_cells':q['total_cells'],'reading_pages':reading_pages,'full_pages':full_pages,'source_hashes_unchanged':q['source_hashes_unchanged']},ensure_ascii=False,indent=2),encoding='utf8')
    (D/'阅读说明.md').write_text(f'''# 药材烘干论文交付

优先阅读“药材烘干论文_正文阅读版.pdf”：共{reading_pages}页，包含摘要、完整四问推导、六张题定结果表、检验与结论、AI声明和参考文献。它省略源代码附录，供修改阅读。

“药材烘干论文_含完整程序附录.pdf”共{full_pages}页，前{reading_pages}页与阅读版内容一致，后附完整数值程序和支撑材料说明。

本次将烘房环境输入整理、时间延续及半径轨迹处理合并为独立“数据预处理”节，置于模型假设与符号说明之后、第一问解答之前。第四问保留比例收缩假设与模型推导，重复的数据处理说明改为引用前文。模型、求解代码和原数值结果未改变。
...
本轮六张题定表{q['total_cells']}个有效数值已与未舍入NPZ独立核对，正文未加入尚未完成的参数、潜热或二维实验。原论文、原数值代码和原结果保留，新稿独立存于paper/guosai2026。
