...
\end{tabular}
\end{table}
前两问覆盖全部规定输出点，后两问还覆盖内部节点，第四问比较至72 h。第二问另以$G_4/G_8$复核，温度、含水率最大差为$6.244\times10^{-7}$ $^\circ$C、$3.732\times10^{-6}$ kg/kg；该数值来自支撑材料的 Q2 数值审计文件（运行后位于\texttt{results/verification/q2/}），可由\texttt{python code/verify\_q2\_results.py --numerical}复现，随附源数组的字段级输出逐点一致。配置差不等于真解误差；临界时间见表\ref{tab_event_convergence}。六张题定表在相应加密下四位一致，但不能据此声称全部Excel单元格的末位均已稳定。
\begin{table}[htbp]\centering\small
...
源数组、题定表及Excel保持同源，数值非有限、含水率越界或必需检查失败均阻断正式输出；另核对第三问准确终时、第四问当前半径及域外标记。这些检查不替代物理验证。
