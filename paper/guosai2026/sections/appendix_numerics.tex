...
\subsection*{B.6\quad 潜热遗漏的条件性量级核算}
本项只对第一问正式$G_8$解作后处理。取$V=\pi R^2L$，仅为质量换算而将820 kg/m$^3$解释为初始湿体积密度，则干物质量$m_d=820V/(1+2.55)$。以环层体积加权的末态均值计算失水$\Delta m_w=m_d(2.55-\bar C)$；再假设失水全部蒸发，取自由水潜热量级$L_v=2.4$ MJ/kg，比较$L_v\Delta m_w$与显热增量$820Vc_p(\bar T-28)$。1800 s时预测失水18.6091 g，对应潜热44.6619 kJ，而显热增量为4.8015 kJ，比值约9.30。该比值提示遗漏机制可能重要，但$L_v$不是该药材的实测参数；纳入潜热后表面温度与对流入热均会变化，因此此后处理不能给出修正解或预测误差界。复现程序与计算记录见支撑材料。
