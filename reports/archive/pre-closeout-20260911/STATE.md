...
- 已选定：LaTeX / xelatex；国赛中文模板工作区入口 `paper/main.tex`（由 `skills/5writing/templates/zh/cumcm-latex` 复制，`fontset=windows`）。
- 编译器：`D:\MikTex\miktex\bin\x64\xelatex.exe`。最小编译通过，输出 `paper/main.pdf`（骨架 6 页，无正文数值）。
- 现有 `paper/drafts/` 未覆盖。各问结果仍待审，正式章节仅占位，不写入未批准数字。
- 本决定不改变第一、二问结果审核状态，也不启动后问求解。
