...
2026-09-13 原图恢复：根据用户对重绘观感的反馈，图4/5/7/11现直接采用其四张原始PNG，保留既有审核与图注中的科学说明。替换版本为 `paper-spacetime-original-v1`，不再引用重绘PDF；详情见 `ORIGINAL_FIGURES_20260913.md`。模型数值和结果表未改。

2026-09-13 时空图采用：用户审核后要求加入正文，第一、二、三问及第四问对照已改为 `paper-spacetime-v1`，从既有正式 NPZ 重绘，无新求解。图型、采样方式和溯源见 `SPACETIME_PAPER_INTEGRATION_20260913.md`、`SPACETIME_FIGURE_PROVENANCE_20260913.json`；旧数值与结果采纳状态保持。

2026-09-13 图形更新：按用户授权，正文全部 10 份结果 PDF 及 3 张示意图已统一参考配色，保留模型数值、图型、坐标与标注，未重跑求解。输入原图、逐图颜色映射、校验和复现方式见 `FIGURE_PALETTE_20260913.md` 与同名 JSON；结果采纳状态不因换色而改变。
