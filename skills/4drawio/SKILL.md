...
先读取 [审核与逐问推进规则](../_references/review_workflow.md) 和 [个人绘图规范](../_references/figure_style.md)。按当前问或已批准整体路线生成图示初稿，与结果一起展示；不另设绘图前置审核，不提前绘制未确定的后问详细方法。
...
以上为按需产物示例，不是最低图数要求。技术路线图只有能解释真实依赖时才生成；某类图不需要时在 `reports/DRAWIO_REPORT.md` 简要说明，已有报告增量更新。
...
- 图中不写大段解释，解释留给论文正文。
- 不使用装饰性阴影和过度渐变。
- 先按真实方法关系组织布局，不默认套固定三栏或固定节点数。参照 `figures/STYLE.md` 中已有选择；没有选择时使用可读初稿，用户认可后再沉淀风格。
- 单图反馈只作用于指定图，明确的全局反馈才同步整套图。纯样式修改保留方法和数据版本；方法关系变化按审核规则处理。
...
按 [实际运行环境与路径](../_references/runtime_environment.md) 复用工具路径。下列 shell 示例属于 Bash；Windows PowerShell 使用已发现的路径，例如 `& $drawioExe --export --format pdf --crop --output $outputPdf $sourceDrawio`，不执行 Bash 的 `command -v` 或 heredoc。
