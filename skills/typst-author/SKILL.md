...
5. **Keep formatting within the authorized edits.** If the formatter would also change unrelated existing code, skip that formatting pass or limit it to the edited region, then continue validation. Reuse an explicit user authorization for broader formatting; do not add a separate approval just for optional formatting.
